...
public class MainActivity extends Activity {
		...
		case R.id.showLectureHalls:
			if (item.isChecked()) {
				...
				for(int i=0; i < result.size(); i++){
					...
					showMarkerOnMap(coords, dao.getName(coords.latitude, coords.longitude));
				}
			}
			return true;
	...
	private void showMarkerOnMap(LatLng lat, String label){
		map.addMarker(new MarkerOptions().position(lat).title(label));
	}
